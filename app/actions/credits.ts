 ...
 */
export async function hasSufficientCredits(userId: string, transaction: UserTransaction): Promise<boolean> {
    const userRef = adminDb.collection('users').doc(userId);
 ...
 */
export async function getUserCredits(userId: string): Promise<{ available: number; used: number }> {
    const userRef = adminDb.collection('users').doc(userId);
    const userDoc = await userRef.get();
    const userData = userDoc.data();
 ...
 */
export async function deductCredits(userId: string, transaction: UserTransaction): Promise<void> {
    const cost = costPerTransaction[transaction];
    if (cost === undefined) {
        throw new Error(`Invalid transaction type: ${transaction}`);
    }
